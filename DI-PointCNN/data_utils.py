...
def load_val_cls(filelist):
    points = []
    labels = []

    for line in open(filelist):
        data = h5py.File(line.rstrip(),'r')
        points.append(data['data'][...].astype(np.float32))
        labels.append(np.squeeze(data['label'][:]).astype(np.int64))
        #data.close()
    return (np.concatenate(points, axis=0),
...
def load_cls_train_val_no_shuffle(filelist, filelist_val):
    data_train, label_train = load_train_cls(filelist)
    data_val, label_val = load_val_cls(filelist_val)
    return data_train, label_train, data_val, label_val
